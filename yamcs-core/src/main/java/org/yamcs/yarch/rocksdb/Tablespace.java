 ...
 * Each corresponds to a directory <tablespace-name>.rdb and has a definition file tablespace-name.tbs.
 * 
 ...
 * 
 * There are two column families in the main database:
 *  - the _metadata_ column family - contains metadata.
 *  - the default column family - contains data.
 *  
 ...
 * 
 * Except for the 4 bytes tbsIndex, the rest of the key and value are completely dependent on the data type. For example for yarch table data, 
 * the rest of key following the 4 bytes tbsIndex represents the key of the row in the table.
 * 
 ...
 *          used to store what information corresponds to each tbsIndex
 *          the record type corresponds to the Type enumerations from tablespace.proto 
...
public class Tablespace {
    ...
    // (basically a table can be moved if the tbsIndex doesn't overlap with another record from the table)
    ...
    private static final byte METADATA_TR = 2; //first byte of metadata records keys that contain tablespace records
    ...
    public void loadDb(boolean readonly) throws IOException, RocksDBException {
        ...
        if(f.exists()) {
            ...
            byte[] value = db.get(METADATA_KEY_MAX_TBS_VERSION);
            if(value[0] != METADATA_VERSION) {
                throw new DatabaseCorruptionException("Wrong metadata version "+value[0]+" expected "+METADATA_VERSION);
            }
    ...
    public List<TablespaceRecord> filter(Type type, String instanceName, Predicate<TablespaceRecord.Builder> p) throws RocksDBException, DatabaseCorruptionException {
        List<TablespaceRecord> r = new ArrayList<>();
        byte[] rangeStart = new byte[] {METADATA_TR, (byte)type.getNumber()};
        
        try (AscendingRangeIterator arit = new AscendingRangeIterator(db.newIterator(cfMetadata), rangeStart, false, rangeStart, false)) {
            while(arit.isValid()) {
     ...
     * 
     * @param trb - the builder has to have all fields set except for the tbsIndex which will be assigned by this method
     * @return a fully built 
     * @throws RocksDBException 
     */
    public TablespaceRecord createRecord(String yamcsInstance, TablespaceRecord.Builder trb) throws RocksDBException {
        if(!trb.hasType()) {
    ...
    private synchronized int getNextTbsIndex() throws RocksDBException {
        ...
        db.put(METADATA_KEY_MAX_TBS_VERSION, v);
    ...
    /**
     * the key to use in the metadata table. We currently use 6 bytes:
     *  1 byte fixed 0xFF
     *  4 bytes tbsIndex
     *   
