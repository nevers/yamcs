...
/**
 * Link implementing the AOS CCSDS 732.0-B-3 for TM
 * 
 ...
 *
 */
public class UdpAosLink extends AbstractScheduledService {
    final String name;
    final String host;
    final int port;
    final int frameSize;
    ...
    // one queue per vc
    private static final Logger log = LoggerFactory.getLogger(UdpAosLink.class);

    int lastVcSent; // switches between 0 and 1 so we don't send always from the same vc

    public UdpAosLink(String name, String host, int port, int frameSize, int framesPerSec) {
        this.name = name;
        this.host = host;
        this.port = port;
